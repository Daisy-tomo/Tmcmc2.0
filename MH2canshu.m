...
%%  1. 参数定义
% 待辨识的敏感参数（仅 eta_k 和 eta_t）
param_names = {'eta_k', 'eta_t'};
...
% 敏感参数真值
theta_true = [0.86, 0.92];

% 固定参数（共 11 个）：eta_T, eta_m 固定在真值，其余 9 个物理参数
theta_fixed = [
    0.98,  ...  % eta_T    涡轮热效率（固定）
    0.995, ...  % eta_m    机械效率（固定）
...
fprintf('===== 固定参数（11 个） =====\n');
fixed_names = {'eta_T','eta_m','lambda','eta_v','eta_tv','eta_c1','eta_c2',...
               'sigma_cc','sigma_kan','sigma_kask','sigma_ks'};
for i = 1:length(theta_fixed)
    fprintf('  %-15s = %.4f\n', fixed_names{i}, theta_fixed(i));
end
fprintf('\n');
...
%% --- 前向模型 ---
% theta_sensitive(1) = eta_k，theta_sensitive(2) = eta_t
% theta_fixed 顺序：eta_T(1), eta_m(2), lambda(3), eta_v(4), eta_tv(5),
%                   eta_c1(6), eta_c2(7), sigma_cc(8), sigma_kan(9),
%                   sigma_kask(10), sigma_ks(11)
...
function plot_results(results, theta_true, lb, ub, param_names)
    ...
    % --- 链轨迹图 ---
    figure('Name', '链轨迹图', 'Position', [50, 50, 1200, 400]);
    for i = 1:n_params
    ...
    legend('链', 'burn-in边界', '真值', 'FontSize', 8, 'Location', 'best');
    sgtitle('MCMC 链轨迹图');

    % --- 边缘后验直方图 ---
    figure('Name', '边缘后验直方图', 'Position', [100, 50, 1000, 420]);
    for i = 1:n_params
        subplot(1, n_params, i);
...
function plot_joint_distributions(results, theta_true, lb, ub, param_names)
    ...
    figure('Name', '两两参数联合分布', 'Position', [200, 50, 700, 650]);
    ...
    for i = 1:n_params
        for j = 1:n_params
            ...
            % 坐标轴标签
            if i == n_params
                xlabel(disp_labels{j}, 'FontSize', 12, 'Interpreter', 'tex');
            end
            if j == 1 && i ~= 1
                ylabel(disp_labels{i}, 'FontSize', 12, 'Interpreter', 'tex', 'Rotation', 90);
            end
    ...
    h4 = plot(ax_leg,  NaN, NaN, '-',  'Color', [0.7,0.7,0.7], 'LineWidth', 1.5);
    legend(ax_leg, [h1, h2, h3, h4], {'MCMC', '先验分布', '后验KDE', '真值'}, ...
        'Orientation', 'horizontal', 'FontSize', 9, 'Location', 'north');

    sgtitle('两两参数联合分布图（\eta_k vs \eta_t）', 'FontSize', 13, 'FontWeight', 'bold', 'Interpreter', 'tex');
end
